-- FUNCTION: public.fn_get_hostellers_sk(text, text, text, text, integer, integer, boolean, text, text)

-- DROP FUNCTION IF EXISTS public.fn_get_hostellers_sk(text, text, text, text, integer, integer, boolean, text, text);

CREATE OR REPLACE FUNCTION public.fn_get_hostellers_sk(
    ...
    RETURNS TABLE(hosteller_id bigint, full_name character varying, email_id character varying, phone_number character varying, dob date, fee numeric, joining_date text, address text, proof text, reason character varying, vacated_date timestamp without time zone, active boolean, total_count bigint) 
    LANGUAGE 'plpgsql'
    COST 100
...
ALTER FUNCTION public.fn_get_hostellers_sk(text, text, text, text, integer, integer, boolean, text, text)
    OWNER TO postgres;
